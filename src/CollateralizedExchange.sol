...
contract CollateralizedExchange {
    ...
    function checkCollateral(
        ...
        uint amountRequired,
        address tokenContract
    ) internal view returns (bool hasEnough, uint amountUnder) {
        uint currentOpenCollateral = openCollateral[checkAddress][
            tokenContract
        ];
        hasEnough = currentOpenCollateral >= amountRequired;
        amountUnder = hasEnough
            ? 0
            : SafeMath.sub(amountRequired, currentOpenCollateral);
        return (hasEnough, amountUnder);
    }

    function addOpenCollateral(
        uint amount,
        address tokenContract
    ) public payable {
        if (tokenContract != address(0x0)) {
            ...
            openCollateral[msg.sender][tokenContract] = openCollateral[
                msg.sender
            ][tokenContract].add(amount);
        } else {
            // Must be native asset
            require(
                msg.value >= amount,
                "Not enough collateral to cover price"
            );
            (bool success, ) = address(this).call{value: msg.value}("");
            require(success, "Failed to receive native asset");
            openCollateral[msg.sender][address(0x0)] = openCollateral[
                msg.sender
            ][address(0x0)].add(msg.value);
        }
    }
    ...
    function lockCollateral(
        ...
        openCollateral[msg.sender][tokenContract] = openCollateral[msg.sender][
            tokenContract
        ].sub(openToSub);
        lockedCollateral[msg.sender][tokenContract] = lockedCollateral[
            msg.sender
        ][tokenContract].add(lockedToAdd);
    }

    function unlockCollateral(uint amount, address tokenContract) internal {
        lockedCollateral[msg.sender][tokenContract] = lockedCollateral[
            msg.sender
        ][tokenContract].sub(amount);
        openCollateral[msg.sender][tokenContract] = openCollateral[msg.sender][
            tokenContract
        ].add(amount);
    }

    function checkAndAddLockedCollateral(
        uint price,
        address tokenContract
    ) internal {
        // Check that collateral has been provided
        (bool hasEnough, uint amountUnder) = checkCollateral(
            msg.sender,
            price,
            tokenContract
        );
        if (tokenContract == address(0x0)) {
            ...
            require(
                amountUnder == msg.value,
                "Reverted because we received too much value"
            );
        }
    ...
    function updateListingPrice(uint itemIndex, uint newPrice) public payable {
        ...
        require(
            ...
            "You cannot update the price of a cancelled listing"
        );
        require(
            item.buyer == address(0x0),
            "Someone has initiated buying this item, you cannot update it"
        );

        if (newPrice > item.price) {
            // Add the difference between existing price and new price
            checkAndAddLockedCollateral(
                SafeMath.sub(newPrice, item.price),
                item.token
            );
            item.price = newPrice;
    ...
    function cancelListing(uint itemIndex) public {
        ...
        require(
            item.buyer == address(0x0),
            "Someone has initiated buying this item, you cannot cancel it"
        );
    ...
    function buyItem(address seller, uint itemIndex) public payable {
        require(
            seller != msg.sender,
            "You cannot buy your own item, try canceling instead"
        );
        ItemListing storage item = itemListings[seller][itemIndex];
        require(
            item.buyer == address(0x0) && !item.isCanceled,
    ...
    function cancelBuy(address seller, uint itemIndex) public {
        require(
            seller != msg.sender,
            "Use the cancelListing method if you want to cancel your own listing"
        );
        ItemListing storage item = itemListings[seller][itemIndex];
        require(
            item.buyer == msg.sender && !item.isSent,
            "Item has already been sent and can no longer be canceled"
        );
        item.buyer = address(0x0);
    ...
    function markItemSent(uint itemIndex) public {
        ...
        require(
            item.buyer != address(0x0),
            "Nobody has bought this item, you can't mark as sent"
        );
    ...
    function markItemReceived(address seller, uint itemIndex) public {
        ...
        lockedCollateral[seller][item.token] = lockedCollateral[seller][
            item.token
        ].sub(item.price);
        // Remove locked collateral from buyer (they locked 2x the item price)
        lockedCollateral[msg.sender][item.token] = lockedCollateral[msg.sender][
            item.token
        ].sub(SafeMath.mul(item.price, 2));
        ...
        openCollateral[seller][item.token] = openCollateral[seller][item.token]
            .add(SafeMath.mul(item.price, 2));
        // Return initial collateral to buyer
        openCollateral[msg.sender][item.token] = openCollateral[msg.sender][
            item.token
        ].add(item.price);
    ...
    function withdrawFunds(uint amount, address tokenContract) public {
        require(
            ...
            "You don't have that amount of funds available"
        );
        if (tokenContract != address(0x0)) {
            IERC20 token = IERC20(tokenContract);
            ...
            openCollateral[msg.sender][address(0x0)] = openCollateral[
                msg.sender
            ][address(0x0)].sub(amount);
        }
    }

    function checkOpenCollateral(
        address addr,
        address tokenContract
    ...
    function getItem(
        address seller,
        uint itemIndex
    ) public view returns (ItemListing memory) {
